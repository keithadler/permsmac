...
enum CLI {
    ...
    static func positional(_ a: [String]) -> [String] {
        ...
        for x in a { if skip { skip = false; continue }; if ["--filter", "--service", "--since"].contains(x) { skip = true; continue }; if x.hasPrefix("--") { continue }; out.append(x) }
        return out
    }
    ...
    static func run(_ cmd: String, _ args: [String]) -> Int32 {
        ...
        case "changes":
            let snap = TCC.read(); let startup = Startup.scan()
            ...
            if js { out(json(["changes": changes.map { ["area": $0.area.rawValue, "what": $0.what.rawValue, "key": $0.key, "before": $0.before ?? "", "after": $0.after ?? "", "when": ISO8601DateFormatter().string(from: $0.when)] }])); return changes.isEmpty ? 0 : 1 }
        ...
        case "startup":
            ...
            if js { out(json(["items": items.map { ["kind": $0.kind.rawValue, "label": $0.label, "program": $0.program, "path": $0.path, "summary": $0.summary] }])); return 0 }
        ...
        case "status":
            let snap = TCC.read(); let records = History.load()
            let d: [String: Any] = ["fullDiskAccess": snap.complete, "userDatabase": snap.userReadable, "systemDatabase": snap.systemReadable, "grants": snap.grants.count, "records": records.count,
                                    "lastLook": records.last.map { ISO8601DateFormatter().string(from: $0.date) } ?? "", "version": version]
