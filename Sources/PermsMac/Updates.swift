...
enum Updates {
    static let repo = "keithadler/permsmac"
    ...
    static func checkAndPresent() {
        Task {
            ...
            case .upToDate:
                ...
                alert.informativeText = String(format: String(localized: "Permissions for Mac %@ is the latest version."), CLI.version)
    ...
    private static func present(version v: String, page: URL) {
        let alert = NSAlert()
        alert.messageText = String(format: String(localized: "Permissions for Mac %@ is available"), v)
