...
#   ./make-dmg.sh            -> build/Permissions-for-Mac-<version>.dmg
...
DMG="build/Permissions-for-Mac-$VERSION.dmg"
STAGE=$(mktemp -d)
...
Permissions for Mac — how to install

1. Drag the "Permissions for Mac" icon onto the "Applications" folder next to it.
...
2. Open the app: open Launchpad (the rocket in the Dock) or your Applications
   folder and click "Permissions for Mac".
...
4. Click the Apple menu (top-left) > System Settings > Privacy & Security.
   Scroll all the way down. Next to "Permissions for Mac was blocked", click
...
5. Done. It opens like any other app from now on, and macOS never asks again.
   When it asks for Full Disk Access, that is how macOS lets it read the list of
   permissions. It never changes anything; it shows you where to.
...
https://github.com/keithadler/permsmac
TXT

rm -f "$DMG"
hdiutil create -volname "Permissions for Mac" -srcfolder "$STAGE" -ov -format UDZO -quiet "$DMG"
