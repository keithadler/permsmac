...
Camera, microphone, screen recording, keyboard, files, contacts, automation, and everything that starts without you. Each one with a sentence that says what an app holding it can actually do, and a button to the exact System Settings pane. It reads and never changes anything.

## Download

**[Download Permissions-for-Mac-1.0.0.dmg](https://github.com/keithadler/permsmac/releases/latest/download/Permissions-for-Mac-1.0.0.dmg)** (macOS 14 or later, Apple Silicon and Intel)
...
- **One list.** Every permission macOS tracks, grouped by what it lets an app do, most sensitive first. Apps you have removed but that still hold a permission are marked.
...
- **A real command line.** `permsmac list`, `permsmac changes --since 7d` (exit 1 when something changed, so it fits in a script), `permsmac startup`, `permsmac orphans`, all with `--json`.
...
- It does not change permissions. Apple only allows that through System Settings, and a tool that reads should not also write.
